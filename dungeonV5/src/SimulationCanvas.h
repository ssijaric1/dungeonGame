...
public:
    ...
    void runMDPAlgorithm() {
        if (!gameState.isGameOver()) {
            std::cout << "MDP: Game must be over to run algorithm" << std::endl;
            return;
        }

        algorithmRunning = true;
        isAnimating = true;
        currentAlgorithm = 6;  // MDP

        const auto& initialState = gameState.getInitialState();

        // MDP uses current gold amount for better policy
        int currentGold = gameState.getGold();

        auto result = DungeonAlgorithms::mdpSearch(
            initialState.actualGrid,
            { initialState.playerStartX, initialState.playerStartY },
            { initialState.exitX, initialState.exitY },
            currentGold  // Pass current gold amount
        );
        ...
        std::cout << "MDP path found with " << fullAlgorithmPath.size()
            << " steps, explored " << fullExploredNodes.size() << " nodes" << std::endl;
...
protected:
    ...
    void resetGame() {
        ...
        // Requirements met or never reached exit yet - generate new dungeon
...
private:
    ...
    void drawAlgorithmDropdown(gui::CoordType x, gui::CoordType y, gui::CoordType width) {
        ...
        case 6: algoName = "MDP (Markov Decision Process)"; break;
        }
    ...
    void drawDropdownMenu(gui::CoordType x, gui::CoordType y, gui::CoordType width) {
        ...
        gui::CoordType menuHeight = 6 * itemHeight;
    ...
    void drawComparisonTable(gui::CoordType x, gui::CoordType y, gui::CoordType width) {
        ...
        if (currentAlgorithm > 0) {
            tableHeight = 200; // Taller to fit algorithm details with generous spacing
        }
        ...
        if (currentAlgorithm > 0) {
            ...
            drawAlgorithmDetails(x + 15, contentY, width - 10);
        }
        else {
    ...
    void drawAlgorithmDetails(gui::CoordType x, gui::CoordType y, gui::CoordType width) {
        ...
        case 5: // Greedy
            ...
            break;
        case 6: // MDP
            algorithmName = "MDP (Markov Decision Process)";
            description = "Finds optimal policy considering uncertainty (mine questions 70% success). Maximizes win probability.";
            heuristic = "Value Iteration with Bellman Equation";
            timeComplexity = "O(|S| * |A| * iterations)";
            spaceComplexity = "O(|S|) where S = states";
            break;
        default:
            return;
        }
