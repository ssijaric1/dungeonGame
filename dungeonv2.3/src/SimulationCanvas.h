...
private:
    // Drawing areas
    gui::CoordType leftZoneLeft, leftZoneTop, leftZoneWidth, leftZoneHeight;
    gui::CoordType rightZoneLeft, rightZoneTop, rightZoneWidth, rightZoneHeight;
    ...
    bool backgroundLoaded = false;  // NEW: Track if background loaded
...
public:
    SimulationCanvas() : rng(std::random_device{}()), gameState(rng) {
        enableResizeEvent(true);
        
        std::cout << "Attempting to load images..." << std::endl;
        
        // Try to load images with try-catch
        try {
            std::cout << "Trying to load player.png..." << std::endl;
            imgPlayer.load("/home/sehy/Work/Common/Icons/images/player.png");
            std::cout << "Success: player.png" << std::endl;
            
            std::cout << "Trying to load reward.png..." << std::endl;
            imgReward.load("/home/sehy/Work/Common/Icons/images/reward.png");
            std::cout << "Success: reward.png" << std::endl;
            
            std::cout << "Trying to load bandit.png..." << std::endl;
            imgBandit.load("/home/sehy/Work/Common/Icons/images/bandit.png");
            std::cout << "Success: bandit.png" << std::endl;
            
            std::cout << "Trying to load mine.png..." << std::endl;
            imgMine.load("/home/sehy/Work/Common/Icons/images/mine.png");
            std::cout << "Success: mine.png" << std::endl;
            
            std::cout << "Trying to load exit.png..." << std::endl;
            imgExit.load("/home/sehy/Work/Common/Icons/images/exit.png");
            std::cout << "Success: exit.png" << std::endl;
            
            std::cout << "Trying to load background.png..." << std::endl;
            imgBackground.load("/home/sehy/Work/Common/Icons/images/background.png");
            std::cout << "Success: background.png" << std::endl;
            ...
            std::cerr << "Exception caught while loading images: " << e.what() << std::endl;
            imagesLoaded = false;
        } catch (...) {
            std::cerr << "Unknown exception caught while loading images!" << std::endl;
            imagesLoaded = false;
        }
        
        if (!imagesLoaded) {
            std::cout << "Images not loaded. Using colored squares as fallback." << std::endl;
        }
        if (!backgroundLoaded) {
            std::cout << "Background image not loaded. Using black background as fallback." << std::endl;
        }
    }
    
    ~SimulationCanvas() = default;

    // Public methods for moving the player
    void moveUp() {
        if (!gameState.gameOver) {
            if (gameState.movePlayer(gameState.playerX, gameState.playerY - 1)) {
                reDraw();
            }
        }
    }
    ...
    void moveLeft() {
        if (!gameState.gameOver) {
            if (gameState.movePlayer(gameState.playerX - 1, gameState.playerY)) {
                reDraw();
            }
        }
    }
    ...
    // Reset game to initial state with new random layout
    void resetGame() {
